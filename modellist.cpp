...
QVariant modelList::data(const QModelIndex &index, int role) const
{
    ...
    if (index.column() == 1)
    {
        if (role == Qt::DisplayRole)
        {
            ...
            a_query.prepare("SELECT ТУчреждение.ИмяУчреждения FROM ТУчреждение WHERE ТУчреждение.КодУчреждения = :val1");
    ...
    if (index.column() == 5)
    {
        if (role == Qt::DisplayRole)
        {
            ...
            QVariant tmp =1.5*QSqlTableModel::data(NewIndex4, role).toDouble() + QSqlTableModel::data(NewIndex5, role).toDouble();
            QVariant got =1.3*tmp.toDouble() + QSqlTableModel::data(NewIndex2, role).toDouble() + QSqlTableModel::data(NewIndex3, role).toDouble();
            return got;
        }
        if (role == Qt::FontRole)
        {
